...
def r_mix(
    ...
    def compute_saliency(model, inputs, labels):
        ...
        target_var = Variable(targets_next).type(torch.cuda.FloatTensor)
        ...
        #         batch_size, 1, 1
        #     )
        #     #saliency = F.interpolate(saliency, size=(inputs.size()), mode='bilinear', align_corners=True) 
    ...
    saliency = compute_saliency(model, inputs, labels)
    #print(saliency.size())
    return saliency
    # with torch.no_grad():
